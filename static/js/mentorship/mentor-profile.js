...
if (!mentorId) {
  ...
  (async () => {
    ...
    if (data.linkedIn) {
      socialDiv.innerHTML += `<a href="${data.linkedIn}" target="_blank" rel="noopener" title="LinkedIn">${LINKEDIN_ICON}</a>`;
    }
    if (data.calendlyUrl) {
      socialDiv.innerHTML += `<a href="${data.calendlyUrl}" target="_blank" rel="noopener" title="תיאום פגישה">📅 תיאום פגישה</a>`;
    }
