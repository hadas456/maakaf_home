...
function renderCard(mentor) {
  ...
  const linkedInLink = mentor.linkedIn
    ? `<a href="${mentor.linkedIn}" target="_blank" rel="noopener" title="LinkedIn">${LINKEDIN_ICON}</a>`
    : '';
  const calendlyLink = mentor.calendlyUrl
    ? `<a href="${mentor.calendlyUrl}" target="_blank" rel="noopener" title="תיאום פגישה">📅 תיאום פגישה</a>`
    : '';
  const socialRow = (linkedInLink || calendlyLink)
    ? `<div class="d-flex gap-2 mt-2 align-items-center">${linkedInLink}${calendlyLink}</div>`
    : '';
